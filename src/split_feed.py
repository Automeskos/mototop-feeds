...
UPDATE_NAME = "inventory-price-update.xml"


def download_feed(url: str, destination: Path) -> dict[str, float | int]:
    started = time.monotonic()
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; Automeskos-Mototop-Feed/1.2)",
...
def current_selling_price(product: etree._Element) -> tuple[float, float]:
    """
    Grąžina:
    - price: dabartinę pardavimo kainą;
    - compare_at_price: seną kainą, kai yra akcija.
    """
    ...
    if special > 0 and regular > special:
        return special, regular

    return regular, 0.0


def update_rows(product: etree._Element) -> list[dict[str, str]]:
    """
    Generuoja tuos pačius SKU, kuriuos naudojome Shopify CSV importui:
    bazinis SKU arba bazinis SKU + option_value_id.
    """
    base_sku = (
    ...
    base_price, base_compare_at = current_selling_price(product)
    ...
    if not option_nodes:
        return [{
            ...
            "compare_at_price": f"{base_compare_at:.2f}" if base_compare_at > 0 else "",
        }]

    # Shopify CSV generatoriuje naudojome pirmą Mototop option grupę.
    ...
    for value in values:
        ...
        if prefix == "-":
            variant_price = max(0.0, base_price - delta)
            variant_compare = max(0.0, base_compare_at - delta) if base_compare_at > 0 else 0.0
        else:
            variant_price = base_price + delta
            variant_compare = base_compare_at + delta if base_compare_at > 0 else 0.0

        rows.append({
            "sku": variant_sku,
            "quantity": str(max(0, safe_int(value.findtext("quantity")))),
            "price": f"{variant_price:.2f}",
            "compare_at_price": f"{variant_compare:.2f}" if variant_compare > 0 else "",
        })
...
def write_update_feed(
    ...
    root = etree.Element(
        ...
        generated_by="Automeskos GitHub stock and price feed",
    )
    ...
    for item in rows:
        ...
        etree.SubElement(product_node, "compare_at_price").text = item["compare_at_price"]
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # SVARBU: likučiai ir kainos generuojami iš VISŲ Mototop produktų,
        # ne tik Aprangos, Detalių ir Aksesuarų.
        ...
        for event, element in context:
            ...
            # Likučiai ir kainos iš viso katalogo.
            all_updates.extend(update_rows(element))
        ...
        generated_at = root_attrs.get(
            "generated_at",
            time.strftime("%Y-%m-%dT%H:%M:%SZ"),
        )

        root_attrs["generated_by"] = "Automeskos GitHub feed splitter"
        ...
        written_updates = write_update_feed(
        ...
        info = {
            **stats,
            ...
            UPDATE_NAME: {
                "skus": written_updates,
                "bytes": (OUTPUT_DIR / UPDATE_NAME).stat().st_size,
            },
        }
